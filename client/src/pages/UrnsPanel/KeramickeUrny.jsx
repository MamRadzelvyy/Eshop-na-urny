...
export default function KeramickeUrny() {
  ...
  useEffect(() => {
    const loadUrns = async () => {
      ...
      if (data.status === 200) {
        const filteredUrns = data.payload.filter((urn) => urn.material === "Keramické urny");
  ...
  return (
    <>
      ...
      <div className="max-w-6xl mx-auto p-6">
        <h1 className="text-4xl font-bold text-center mb-6">Keramické urny</h1>
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-6">
          {sortedUrns.length > 0 ? (
            ...
            <p className="text-center text-gray-500 col-span-full">Žádné keramické urny nebyly nalezeny.</p>
          )}
        </div>
      </div>
      <Footer />
