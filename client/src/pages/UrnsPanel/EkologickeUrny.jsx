...
export default function EkologickeUrny() {
  ...
  useEffect(() => {
    const loadUrns = async () => {
      ...
      if (data.status === 200) {
        const filteredUrns = data.payload.filter((urn) => urn.material === "Ekologické urny");
...
<div className="flex justify-center gap-4 mt-4">
  ...
  <Button className="px-7 py-3 bg-green-200 text-green-900 hover:bg-green-300">
    <ShoppingCart className="w-5 h-5" />
            ...
            <p className="text-center text-gray-500 col-span-full">Žádné ekologické urny nebyly nalezeny.</p>
          )}
        </div>
      </div>
      <Footer />
