...
export default function VelkeZvireciUrny() {
    ...
    const loadUrns = async () => {
      ...
      if (data.status === 200) {
        const filtered = data.payload.filter(
          (urn) => urn.material === "Zvířecí urny L"
        );
        setUrns(filtered);
      }
    };
  ...
  return (
    <>
      ...
      <div className="max-w-6xl mx-auto p-6">
        <h1 className="text-4xl font-bold text-center mb-6">Zvířecí urny – velikost L</h1>
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-6">
          {sortedUrns.length > 0 ? (
                ...
                <Card className="shadow-lg rounded-xl overflow-hidden">
                  ...
                  <CardContent className="p-4 text-center">
                    ...
                    <Link to={`/urny/${urn._id}`}>
                      <Button className="mt-3 w-full bg-gray-700 text-white">
                        Zobrazit detaily
                      </Button>
                    </Link>
                  </CardContent>
            ...
            <p className="text-center text-gray-500 col-span-full">
              Žádné velké zvířecí urny nebyly nalezeny.
            </p>
          )}
        </div>
      </div>
